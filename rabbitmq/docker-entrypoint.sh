...
if [[ $1 == rabbitmq ]] && [ $(id -u) = '0' ]; then
	if [ $1 = 'rabbitmq-server' ]; then
		chown -R rabbitmq varlibrabbitmq
	fi
...
# httpswww.rabbitmq.comconfigure.html
sslConfigKeys=(
	cacertfile
	certfile
	depth
...
if [ ${RABBITMQ_ERLANG_COOKIE-} ]; then
	cookieFile='varlibrabbitmq.erlang.cookie'
	if [ -e $cookieFile ]; then
		if [ $(cat $cookieFile 2devnull) != $RABBITMQ_ERLANG_COOKIE ]; then
			echo &2
...
configBase=${RABBITMQ_CONFIG_FILE-etcrabbitmqrabbitmq}
...
if [ -n $shouldWriteConfig ] && [ -f $oldConfigFile ]; then
	{
		...
		echo    - convert '$oldConfigFile' to the newer sysctl format ('$newConfigFile'); see httpswww.rabbitmq.comconfigure.html#config-file
	} &2
	exit 1
fi
...
# httpstackoverflow.coma2705678433558
sed_escape_lhs() {
	echo $@  sed -e 's[]$.^[]&g'
}
sed_escape_rhs() {
...
rabbit_env_config() {
	...
	for conf; do
		...
		local rawVal=$val
		case $conf in
			fail_if_no_peer_certhipe_compile)
				case ${val,,} in
					falseno0'') rawVal='false' ;;
					trueyes1) rawVal='true' ;;
				esac
				;;
...
if [ $1 = 'rabbitmq-server' ] && [ $shouldWriteConfig ]; then
	...
	memTotalKb=
	if [ -r procmeminfo ]; then
		memTotalKb=$(awk -F ' +' '$1 == MemTotal { print $2; exit }' procmeminfo)
	fi
	memLimitB=
	if [ -r sysfscgroupmemorymemory.limit_in_bytes ]; then
		...
		}' sysfscgroupmemorymemory.limit_in_bytes)
	fi
	if [ -n $memLimitB ]; then
		...
		# httpsgithub.comrabbitmqrabbitmq-serverpull1234
		rabbit_set_config 'total_memory_available_override_value' $memLimitB
	fi
	# httpswww.rabbitmq.commemory.html#memsup-usage
	if [ ${RABBITMQ_VM_MEMORY_HIGH_WATERMARK-} ]; then
		# httpsgithub.comdocker-libraryrabbitmqpull105#issuecomment-242165822
			...
			echo $RABBITMQ_VM_MEMORY_HIGH_WATERMARK  awk '
				^[0-9][.][0-9]+$^[0-9]+([.][0-9]+)%$ {
					...
					if (perc  1.0  perc = 0.0) {
						printf error invalid percentage for vm_memory_high_watermark %s (must be  0%%, = 100%%)n, $0  devstderr;
						exit 1;
					}
					printf vm_memory_high_watermark.relative %0.03fn, perc;
					next;
				}
				^[0-9]+$ {
					printf vm_memory_high_watermark.absolute %sn, $0;
					next;
				}
				^[0-9]+([.][0-9]+)[a-zA-Z]+$ {
					printf vm_memory_high_watermark.absolute %sn, $0;
				...
				{
					printf error unexpected input for vm_memory_high_watermark %sn, $0;
					exit 1;
				}
			'
		)
	...
	# httpswww.rabbitmq.commanagement.html#configuration
	if [ $(rabbitmq-plugins list -m -e rabbitmq_management) ]; then
		...
		# httpswww.rabbitmq.commanagement.html#load-definitions
		managementDefinitionsFile='etcrabbitmqdefinitions.json'
		if [ -f $managementDefinitionsFile ]; then
			# see also httpsgithub.comdocker-libraryrabbitmqpull112#issuecomment-271485550
			rabbit_set_config 'management.load_definitions' $managementDefinitionsFile
		fi
	fi
fi

combinedSsl='tmpcombined.pem'
...
if [ $haveSslConfig ] && [ -f $combinedSsl ]; then
	...
	export ERL_SSL_PATH=$(erl -eval 'ioformat(~p, [codelib_dir(ssl, ebin)]),halt().' -noshell)
